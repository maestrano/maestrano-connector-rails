module Maestrano::Connector::Rails::Concerns::Entity
  ...
  module ClassMethods
    ...
    def push_entities_to_connec_by_name(connec_client, connec_entity_name, external_entity_name, mapped_external_entities_with_idmaps)
      Rails.logger.info "Push #{@@external_name} #{external_entity_name.pluralize} to Connec! #{connec_entity_name.pluralize}"
      mapped_external_entities_with_idmaps.each do |mapped_external_entity_with_idmap|
        ...
        if idmap.connec_id.blank?
          connec_entity = Entity.create_entity_to_connec_by_name(connec_client, connec_entity_name, external_entity)
          ...
          connec_entity = Entity.update_entity_to_connec_by_name(connec_client, connec_entity_name, external_entity, idmap.connec_id)
          idmap.update_attributes(last_push_to_connec: Time.now)
        end
      end
    end

    def create_entity_to_connec_by_name(connec_client, connec_entity_name, external_entity)
      Rails.logger.info "Create #{connec_entity_name}: #{mapped_external_entity} to Connec!"
      response = client.post("/#{connec_entity_name.downcase.pluralize}", { "#{connec_entity_name.downcase.pluralize}".to_sym => mapped_external_entity })
      JSON.parse(response.body)["#{connec_entity_name.downcase.pluralize}"]
    end

    def update_entity_to_connec_by_name(connec_client, connec_entity_name, external_entity, connec_id)
      Rails.logger.info "Update #{connec_entity_name}: #{mapped_external_entity} to Connec!"
      client.put("/#{connec_entity_name.downcase.pluralize}/#{connec_id}", { "#{connec_entity_name.downcase.pluralize}".to_sym => mapped_external_entity })
    end
    ...
    def push_entities_to_external_by_name(external_client, external_entity_name, connec_entity_name, mapped_connec_entities_with_idmaps)
      Rails.logger.info "Push Connec! #{connec_entity_name.pluralize} to #{@@external_name} #{external_entity_name.pluralize}"
      mapped_connec_entities_with_idmaps.each do |mapped_connec_entity_with_idmap|
        Entity.push_entity_to_external_by_name(external_client, external_entity_name, mapped_connec_entity_with_idmap)
      end
    end

    def push_entity_to_external_by_name(external_client, external_entity_name, mapped_connec_entity_with_idmap)
      ...
      if idmap.external_id.blank?
        external_id = Entity.create_entity_to_external_by_name(external_client, external_entity_name, connec_entity)
        idmap.update_attributes(external_id: external_id, external_entity: external_entity_name, last_push_to_external: Time.now)
      else
        Entity.update_entity_to_external_by_name(external_client, external_entity_name, connec_entity, idmap.external_id)
        idmap.update_attributes(last_push_to_external: Time.now)
      end
    end

    def create_entity_to_external_by_name(client, external_entity_name, mapped_connec_entity)
      Rails.logger.info "Create #{external_entity_name}: #{mapped_connec_entity} to #{@@external_name}"
      raise "Not implemented"
    end

    def update_entity_to_external_by_name(client, external_entity_name, mapped_connec_entity, external_id)
  ...
  def get_connec_entities(client, last_synchronization, opts={})
    Entity.get_connec_entities_by_name(client, last_synchronization, self.connec_entity_name, opts)
  end

  def push_entities_to_connec(connec_client, mapped_external_entities_with_idmaps)
    Entity.push_entities_to_connec_by_name(connec_client, self.connec_entity_name, self.external_entity_name, mapped_external_entities_with_idmaps)
  end
  ...
  def get_external_entities(client, last_synchronization, opts={})
    Entity.get_external_entities_by_name(client, last_synchronization, self.external_entity_name, opts)
  end

  def push_entities_to_external(external_client, mapped_connec_entities_with_idmaps)
    Entity.push_entities_to_external_by_name(external_client, self.external_entity_name, self.connec_entity_name, mapped_connec_entities_with_idmaps)
  end
  ...
  def consolidate_and_map_data(connec_entities, external_entities, organization)
    external_entities.map!{|entity|
      idmap = IdMap.find_by(external_id: Maestrano::Connector::Rails::External.get_id_from_entity_hash(entity), external_entity: self.external_entity_name.downcase, organization_id: organization.id)

      # No idmap: creating one, nothing else to do
      unless idmap
        next {entity: self.map_to_connec(entity), idmap: IdMap.create(external_id: Maestrano::Connector::Rails::External.get_id_from_entity_hash(entity), external_entity: self.external_entity_name.downcase, organization_id: organization.id)}
      end

      # Entity has not been modified since its last push to connec!
      if idmap.last_push_to_connec && idmap.last_push_to_connec > Maestrano::Connector::Rails::External.get_last_update_date_from_entity_hash(entity)
      ...
      if idmap.connec_id && connec_entity = connec_entities.detect{|connec_entity| connec_entity['id'] == idmap.connec_id}
        ...
        if connec_entity['updated_at'] < Maestrano::Connector::Rails::External.get_last_update_date_from_entity_hash(entity)
